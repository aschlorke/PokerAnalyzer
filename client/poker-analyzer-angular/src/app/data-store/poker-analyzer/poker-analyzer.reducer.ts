...
export const pokerAnalyzerReducer = createReducer(
  ...
  on(pokerAnalyzerActions.DeleteGameSuccess, (state, action) => {
    const currentIds = [...state.existingGameIds].filter(
      (id) => id !== action.id
    );
...
export const selectPokerAnalyzerState = createFeatureSelector<
  AppState,
  PokerAnalyzerState
>(pokerAnalyzerStateKey);
export const getCurrentNumberOfPlayers = createSelector(
  selectPokerAnalyzerState,
  (pokerAnalyzerState: PokerAnalyzerState) =>
    pokerAnalyzerState.currentNumberOfPlayers
);
