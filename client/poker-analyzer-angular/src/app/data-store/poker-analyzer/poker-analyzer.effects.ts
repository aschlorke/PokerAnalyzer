...
import {
  ...
  getExistingGameIds,
} from './poker-analyzer.reducer';
import { map } from 'rxjs';
import { AppState } from 'src/app/app-state';
import { PokerGamesService } from 'src/app/services/poker-games.service';
...
export class PokerAnalyzerEffects {
  constructor(
    ...
    private _analyzerService: PokerGamesService
  ) {}
    ...
    () =>
      this._actions$.pipe(
        ...
        map(([action, numPlayers]) => {
          return this._analyzerService
            .getNewPokerGame(numPlayers)
            .pipe(take(1))
            .subscribe((pokerGame) => {
              this._store.dispatch(
                pokerAnalyzerActions.SetCurrentGameSuccess({ pokerGame })
              );
  ...
  getExistingGame$ = createEffect(
    () =>
      this._actions$.pipe(
        ...
        map((action) => {
          return this._analyzerService
            .getExistingPokerGame(action.id)
            .pipe(take(1))
            .subscribe((pokerGame) =>
              this._store.dispatch(
                pokerAnalyzerActions.SetCurrentGameSuccess({ pokerGame })
  ...
  getExistingIds$ = createEffect(
    () =>
      this._actions$.pipe(
        ...
        map((action) => {
          return this._analyzerService
            .getExistingPokerGameIds()
            .pipe(take(1))
            .subscribe((ids) =>
              this._store.dispatch(
                pokerAnalyzerActions.GetExistingIdsSuccess({ ids })
              )
            );
        })
      ),
  ...
  deleteGame$ = createEffect(
    () =>
      this._actions$.pipe(
        ...
        map(([action, ids]) => {
          return this._analyzerService
            .deleteGame(action.id)
            .pipe(
              take(1),
              map((success) => {
                if (success) {
                  this._store.dispatch(
                    pokerAnalyzerActions.DeleteGameSuccess({ id: action.id })
                  );
                  ...
                  if (!differentIds.length) {
                    this._store.dispatch(
                      pokerAnalyzerActions.SetCurrentGameSuccess(null)
                    );
                  } else {
                    this._store.dispatch(
                      pokerAnalyzerActions.SetCurrentGame({
                        id: differentIds[0],
                      })
                    );
                  }
